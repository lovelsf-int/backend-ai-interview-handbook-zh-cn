...
- 新增八道 P7/P8 架构决策实战题：逐题提供标准回答、深挖回答、追问和验收证据，加入容量算例、架构决策模板与训练评分边界。

- 新增 P7/P8 面试冲刺与能力缺口地图，统一历次复盘入口、P0/P1 优先级、七天训练路线和上场检查单。
- 补齐 Java 集合、Map、泛型擦除与不可变集合的高频追问，明确 API 契约与具体 JDK 实现细节的边界。
- 补齐 JVM 类加载、链接、初始化、对象分配、GC Roots、类卸载与 Metaspace 泄漏链路。
- 补齐 MySQL Performance Schema、执行计划漂移、Online DDL、元数据锁、复制延迟和大表变更 Runbook。
- 将新增页面接入首页、学习路线、顶部导航、专题首页与分类侧栏，并增加公开路由验收。
