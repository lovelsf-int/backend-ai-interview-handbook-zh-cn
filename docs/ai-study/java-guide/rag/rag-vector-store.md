---
title: "RAG 向量索引算法和向量数据库 · 学习版"
description: "向量索引与数据库：许可正文与个人复习"
status: "reviewing"
...
source_blob: "06ac1ea3b3e48e231efb47e5c6005f53fa65f277"
source_url: "https://javaguide.cn/ai/rag/rag-vector-store.html"
---

# RAG 向量索引算法和向量数据库

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/rag/rag-vector-store.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/rag/rag-vector-store.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
向量存储不等于 ANN 检索能力。索引选择要同时考虑召回、过滤、延迟、更新、内存和运维，而不只看条数。

### 在脑中走一遍链路

选择 Embedding 与距离度量 → 建精确检索基线 → 建候选索引 → 带过滤查询 → 比较召回/延迟/资源 → 运维与更新评估。

### 容易记错的边界

提高候选量不保证端到端答案更好；相同维度的两个模型也不代表处在兼容的向量空间。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜HNSW 为什么是近似检索？</summary>

它通过图上的候选探索寻找近邻，不保证遍历全部向量；用较少计算换取检索效率，需要用精确检索或标注集衡量召回损失。

</details>

<details>
<summary>P7｜过滤后 Top K 不足怎么办？</summary>

核对权限与过滤范围，再检查候选生成和过滤发生位置、索引能力与查询参数；在成本预算内扩大搜索或使用受控兜底，不能取消权限过滤补齐数量。

</details>

<details>
<summary>P8｜已有 ES 还要不要引入专用向量库？</summary>

先按真实维度、过滤比例、更新率、QPS 和混合检索需求评测现有 ES；只有独立扩展、隔离或专用能力收益覆盖新增运维与一致性成本时再拆。

</details>

### 动手或纸上推演

设计一个带租户过滤的向量检索压测表：精确基线、召回率、P95、内存、构建时间与更新成本分别记录。
...
基础概念可以看 [RAG 基础篇](/ai-study/java-guide/rag/rag-basis.md)。本文重点放在后半段：这些向量怎么高效存储、索引和检索。
...
[原图：向量索引算法分类](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-vector-index-algorithms-Bjze1jhj.png)
...
[原图：HNSW 索引架构](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-hnsw-architecture.png)
...
[原图：VECTOR 列不能用作任何类型的键，包括主键、外键、唯一键和分区键](https://oss.javaguide.cn/github/javaguide/ai/rag/mysql9-vector-cannot-be-used-as-any-type-of-key.png)
