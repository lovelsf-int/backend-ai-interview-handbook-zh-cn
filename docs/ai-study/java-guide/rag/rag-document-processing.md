---
title: "RAG 文档处理与切分策略：从解析、清洗、Chunking 到多模态内容处理 · 学习版"
description: "RAG 文档处理与切分：许可正文与个人复习"
status: "reviewing"
...
source_blob: "ece7426173a43f66df1c5234ee5dbea90a19ea7c"
source_url: "https://javaguide.cn/ai/rag/rag-document-processing.html"
---

# RAG 文档处理与切分策略：从解析、清洗、Chunking 到多模态内容处理

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/rag/rag-document-processing.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/rag/rag-document-processing.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
切分首先服务语义完整和证据定位，再讨论长度与重叠。文档类型、标题结构、表格及权限必须进入设计。

### 在脑中走一遍链路

解析原文 → 清洗但保留证据 → 识别结构 → 切分与父子关系 → 继承元数据 → 索引 → 抽样检索评测。

### 容易记错的边界

固定字符数不等于固定 Token；重叠过多会重复召回和挤占上下文，表格按行硬切可能丢掉表头语义。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜每个 Chunk 至少带哪些信息？</summary>

文档 ID、版本、位置、标题路径、来源类型、时间和访问范围；根据业务保存父块引用、内容摘要和解析版本，确保可回溯。

</details>

<details>
<summary>P7｜制度、FAQ 和案例怎么分别切？</summary>

制度优先保持条款及条件，FAQ 保持问答关联，案例保留背景、证据、动作与结果关系；长内容可以子块检索、父块补全，但仍受预算和权限约束。

</details>

<details>
<summary>P8｜如何证明换切分策略有效？</summary>

固定文档与查询集，对比不同切法的证据召回、重复率、上下文利用、答案质量、索引成本和延迟，使用消融而不是只看单个好例子。

</details>

### 动手或纸上推演

手动切分一份含表格和例外条款的 SOP，给每个块写标题路径和定位信息，测试能否回答含否定条件的问题。

深入对照：[本站 P7/P8 工程章节](/ai-agent/05-rag-knowledge-engineering.md)。官方/论文延伸：[论文：Retrieval-Augmented Generation](https://arxiv.org/abs/2005.11401)。
...
[原图：语义丢失：上下文依赖关系被切碎](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-document-processing-semantic-loss.png)
...
[原图：结构丢失问题：不同格式，坑完全不一样](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-document-processing-structure-loss.png)
...
[原图：分层校验策略：没有质检的管线，不是生产级管线](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-document-processing-hierarchical-verification-strategy.png)
...
[原图：如何从零搭一套企业级文档处理管线？](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-document-processing-build-enterprise-document-processing-pipeline-from-scratch.png)
