---
title: "RAG 优化：从召回、重排到上下文工程 · 学习版"
description: "RAG 检索优化：许可正文与个人复习"
status: "reviewing"
...
source_blob: "b5e671ff5b045882b2d1c8bc01d8562dc2eff048"
source_url: "https://javaguide.cn/ai/rag/rag-optimization.html"
---

# RAG 优化：从召回、重排到上下文工程

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/rag/rag-optimization.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/rag/rag-optimization.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
按召回、融合、重排和上下文逐层定位问题。每增加一个优化步骤，都要证明质量收益值得它的延迟与成本。

### 在脑中走一遍链路

查询理解 → BM25/向量等召回 → 去重融合 → 重排 → 上下文选择 → 生成 → 分层评测。

### 容易记错的边界

RRF 融合排名，不代表概率；Embedding 模型不应未经核验就当成专用 Reranker。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜为什么 BM25 和向量检索可以互补？</summary>

关键词检索利于精确名称、代码和术语匹配，向量检索利于语义相关表达；互补程度仍取决于语料、查询和模型，需评测确认。

</details>

<details>
<summary>P7｜Recall@K 很高但答案不好怎么办？</summary>

检查相关证据是否被排序挤掉、重复内容占满上下文、引用被压缩丢失，或模型没有遵守证据。不要继续无条件扩大 K。

</details>

<details>
<summary>P8｜怎样给 Query Rewrite 和 Rerank 做消融？</summary>

保持文档、查询集和生成模型不变，逐一启停步骤，比较分层质量、尾延迟、调用成本以及失败样本，避免多项同时修改后无法归因。

</details>

### 动手或纸上推演

给一组告警构造精确 IOC 查询与语义描述查询，分别比较关键词、向量和混合检索的优劣；测试数据仅用合成内容。
