---
title: "RAG 基础概念：检索、生成与工程取舍 · 学习版"
description: "RAG 基础：许可正文与个人复习"
status: "reviewing"
...
source_blob: "9ca99533053fd7481a123dbe44b98a68f968550d"
source_url: "https://javaguide.cn/ai/rag/rag-basis.html"
---

# RAG 基础概念：检索、生成与工程取舍

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/rag/rag-basis.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/rag/rag-basis.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
RAG 在生成前检索外部证据，改进知识获取与可追溯性；它并不会自动消除错误检索、证据冲突和幻觉。

### 在脑中走一遍链路

离线：解析 → 切分 → 建索引；在线：问题 → 检索 → 排序 → 上下文 → 有依据的回答与引用。

### 容易记错的边界

新告警通常是查询或结构化事件，不是必须像长知识文档那样切片；长附件可另走文档处理。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜RAG 和模型直接回答最大的边界差别是什么？</summary>

RAG 增加可更新、可追溯的外部证据通道，但生成结果仍需检查证据是否支持结论。检索不到时应承认不足，而不是补造答案。

</details>

<details>
<summary>P7｜制度库与案例库为什么要区别对待？</summary>

制度描述规则及生效版本，案例提供相似事件与结果，不能让旧案例覆盖当前正式制度。两类资料分别检索并保留来源类型。

</details>

<details>
<summary>P8｜RAG 链路怎么建立完整质量指标？</summary>

分别衡量资料有效性、可访问性、召回、排序、上下文保留、引用支撑与任务结果，并把缺少证据与模型误用证据区分开。

</details>

### 动手或纸上推演

构造一个短告警、一份长 SOP 和一个历史工单，标出谁作为查询、谁需要切片，以及引用需要哪些元数据。
...
[原图：RAG 示意图](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-index-and-retrieval-explainer.webp)

## 为什么需要 RAG？

[原图：RAG（检索增强生成）如何解决 LLM 的核心挑战](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-llm-challenges.png)
...
检索效果不稳定时，问题往往出在查询改写、召回策略、排序或上下文质量上。优化方向可以看 [RAG 优化篇](/ai-study/java-guide/rag/rag-optimization.md)。
...
[原图：RAG 与传统搜索引擎的区别](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-rag-vs-search-engine.png)
...
[原图：RAG 演进阶段](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-2-evolution-stages.png)
...
Naive RAG 是起点，能跑通 Demo，但离生产通常还有距离。Advanced RAG 开始处理召回质量、噪声过滤和排序问题。Modular RAG 把各环节拆成可替换模块，更适合复杂场景。具体优化策略可以继续看 [RAG 优化篇](/ai-study/java-guide/rag/rag-optimization.md)。
