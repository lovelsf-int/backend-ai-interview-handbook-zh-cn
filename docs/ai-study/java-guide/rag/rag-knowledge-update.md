---
title: "RAG 知识库文档如何更新：增量更新、版本控制、去重与全量重建 · 学习版"
description: "RAG 知识更新：许可正文与个人复习"
status: "reviewing"
...
source_blob: "1817ef4b5a89efda9ee3b78104af8b8d207ef469"
source_url: "https://javaguide.cn/ai/rag/rag-knowledge-update.html"
---

# RAG 知识库文档如何更新：增量更新、版本控制、去重与全量重建

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/rag/rag-knowledge-update.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/rag/rag-knowledge-update.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
知识更新是原文、切片、向量、权限、缓存和引用之间的一致性问题。需要版本、幂等、失败恢复与删除传播。

### 在脑中走一遍链路

检测源变更 → 计算文档版本 → 解析切分 → 生成新索引数据 → 验证 → 切换可见版本 → 清理旧副本。

### 容易记错的边界

删除原文不代表向量和缓存已删除；只更新一部分 Chunk 可能让同一答案混用两版 SOP。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜怎样判断文档是否需要重新向量化？</summary>

结合内容摘要与解析、切分、Embedding 版本；只改无关元数据未必需要重算向量，但权限变化仍须立即影响可见性。

</details>

<details>
<summary>P7｜新版本导入到一半失败怎么办？</summary>

保持已验证版本对外服务，未完成版本不可部分冒充完整更新；用任务状态和幂等重试补齐，验证后切换，并能回滚。

</details>

<details>
<summary>P8｜更换 Embedding 模型如何迁移？</summary>

建立明确的新索引或向量空间版本，重新生成文档与查询向量，双路评测后切换；同维度也不能直接混用新旧模型产生的向量。

</details>

### 动手或纸上推演

设计 SOP 从 v1 更新到 v2 的状态机，包含部分写入失败、重复事件、撤权与回滚四条异常路径。
