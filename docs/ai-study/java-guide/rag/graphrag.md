---
title: "GraphRAG：用图结构补充向量检索 · 学习版"
description: "GraphRAG：许可正文与个人复习"
status: "reviewing"
...
source_blob: "bccbb9d0eef2fba59d493d2cb1b1874e4fd043c5"
source_url: "https://javaguide.cn/ai/rag/graphrag.html"
---

# GraphRAG：用图结构补充向量检索

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/rag/graphrag.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/rag/graphrag.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
先明确问题是否需要关系和跨文档聚合，再评估图抽取、检索及摘要。GraphRAG 不是把向量库替换成图库那么简单。

### 在脑中走一遍链路

实体与关系抽取 → 消歧与来源关联 → 图或社区结构 → 查询匹配 → 证据子图/摘要 → 有来源的生成。

### 容易记错的边界

抽取出来的边不是已证实事实；图摘要也可能继承抽取错误，不能因为有图结构就提高可信等级。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜什么问题更值得尝试 GraphRAG？</summary>

跨文档关联、多跳关系或全局主题汇总可以成为候选场景；直接事实查询未必需要额外图构建成本。不同 GraphRAG 实现能力不应混为一谈。

</details>

<details>
<summary>P7｜SOC 实体同名或 IP 复用怎么处理？</summary>

结合租户、时间、稳定资产标识和来源进行实体解析，保留歧义与证据，而不是仅凭字符串相同合并节点。

</details>

<details>
<summary>P8｜如何验证图方案的增量价值？</summary>

与同预算的普通混合 RAG 对照，覆盖多跳和简单查询，统计关系正确性、引用支撑、更新成本、延迟及任务结果，再决定是否扩大使用。

</details>

### 动手或纸上推演

用三份合成工单画出实体—关系—证据表，故意加入一个同名不同资产，说明如何避免错误合并。

深入对照：[本站 P7/P8 工程章节](/ai-agent/05-rag-knowledge-engineering.md)。官方/论文延伸：[Microsoft GraphRAG 文档](https://microsoft.github.io/graphrag/)。
...
[原图：什么是 RAG？](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-index-and-retrieval-explainer.webp)
...
[原图：什么是 GraphRAG？](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-knowledge-relationship-explainer.webp)
...
[原图：传统向量 RAG 的局限性](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-vector-rag-limitation.png)
...
[原图：GraphRAG 和传统向量 RAG 的区别](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-vs-rag.png)
...
[原图：GraphRAG 的核心概念](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-core-concept.png)
...
[原图：GraphRAG 索引流程](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-build-process.png)
...
[原图：GraphRAG 查询阶段：先判断问题类型](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-query-routing.png)
