---
title: "大模型结构化输出：从 JSON 契约到 Function Calling 落地 · 学习版"
description: "结构化输出与 Function Calling：许可正文与个人复习"
status: "reviewing"
...
source_blob: "bd0bd35e87a088baefe5fc0b7b9009d0917230c4"
source_url: "https://javaguide.cn/ai/llm-basis/structured-output-function-calling.html"
---

# 大模型结构化输出：从 JSON 契约到 Function Calling 落地

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/llm-basis/structured-output-function-calling.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/llm-basis/structured-output-function-calling.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
区分返回格式、工具调用意图和实际执行。Schema 可以限制结构，不能代替资源权限、业务规则和副作用控制。

### 在脑中走一遍链路

定义工具 Schema → 模型返回名称与参数 → 应用解析 → 认证与参数/资源校验 → 调用工具 → 回传关联结果 → 模型继续。

### 容易记错的边界

不要接收模型生成的 isAdmin、approved 或 tenantId 作为授权依据；JSON 合法也可能对应错误资产。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜Function Calling 与普通 JSON 输出有何不同？</summary>

前者表达选择哪个工具及参数，后者表达某种结构化结果；应用都需要按提供方协议解析。是否执行以及怎样执行始终由应用负责。

</details>

<details>
<summary>P7｜模型给出的资产 ID 不属于当前租户怎么办？</summary>

从认证上下文确定租户，再查权威资产归属；拒绝越权提议，且不在错误响应里泄露另一租户的对象信息。不能让模型自己确认权限。

</details>

<details>
<summary>P8｜换模型后工具调用兼容性如何验证？</summary>

固定工具契约、模型版本和任务集，覆盖缺字段、错误类型、未知工具、并行调用、重复调用和拒绝；分别验证结构通过率与真实业务完成率。

</details>

### 动手或纸上推演

给只读资产查询工具编写 Schema，再列出五个 Schema 合法但业务上必须拒绝的请求。

深入对照：[本站 P7/P8 工程章节](/ai-agent/04-tools-mcp-a2a.md)。官方/论文延伸：[Spring AI：Tool Calling](https://docs.spring.io/spring-ai/reference/api/tools.html)。
...
本节重点介绍工具参数和执行约束。如果需要把间接 Prompt Injection、MCP 授权、敏感数据、网络出口、代码隔离与安全评测串起来，可以继续看 [LLM/Agent 安全实战](/ai-study/java-guide/system-design/llm-security.md)。
