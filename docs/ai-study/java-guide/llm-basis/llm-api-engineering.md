---
title: "大模型 API 调用工程实践：流式输出、重试、限流与结构化返回 · 学习版"
description: "大模型 API 调用工程：许可正文与个人复习"
status: "reviewing"
...
source_blob: "52a8498e2286d8f26ecbce49835684b8bc58e112"
source_url: "https://javaguide.cn/ai/llm-basis/llm-api-engineering.html"
---

# 大模型 API 调用工程实践：流式输出、重试、限流与结构化返回

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/llm-basis/llm-api-engineering.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/llm-basis/llm-api-engineering.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
一次模型请求要放进完整的超时、限流、取消、重试和业务幂等链路，而不是只写一个 HTTP POST。

### 在脑中走一遍链路

请求校验 → 租户配额 → 上下文预算 → 模型调用 → 流式组装 → 结果校验 → 业务状态更新与观测。

### 容易记错的边界

读取流的某个片段不能视为完整结果；客户端取消不一定意味着模型或外部动作已经停止。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜流式返回的 delta 能直接当完整 JSON 解析吗？</summary>

不能把任意传输片段当完整业务对象。应按提供方事件协议组装，处理结束、拒绝、截断和错误，再校验结构与业务语义。

</details>

<details>
<summary>P7｜429、超时和参数错误怎么重试？</summary>

按错误语义区分：参数错误先修正；限流遵循服务端退避提示并受总时限约束；超时先判断是否可能已有副作用。重试次数、抖动和预算由调用层统一控制。

</details>

<details>
<summary>P8｜多层 SDK 都重试会怎样？</summary>

重试乘法放大流量和成本。应定义唯一或协调的重试责任层、贯穿请求的 deadline、累计尝试预算，并观测原始请求与尝试请求之比。

</details>

### 动手或纸上推演

用本地模拟接口注入限流、慢响应、半途断流和取消，设计每种状态的客户端表现与最终业务状态。

深入对照：[本站 P7/P8 工程章节](/ai-agent/09-production-reliability-cost.md)。官方/论文延伸：[Spring AI：Tool Calling](https://docs.spring.io/spring-ai/reference/api/tools.html)。
...
上文默认你理解 Token、上下文窗口、Temperature、Top-p 等基础概念。如果还有疑问，建议先看[《LLM 运行机制：Token、上下文窗口与采样参数怎么影响输出》](/ai-study/java-guide/llm-basis/llm-operation-mechanism.md)和[《大模型提示词工程（Prompt Engineering）是什么？提示词技巧有哪些？》](/ai-study/java-guide/agent/prompt-engineering.md)。
...
[原图：SSE、WebSocket 与 HTTP chunked 的流式协议选型对比](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-api-engineering-streaming-protocols.webp)
...
[原图：流式调用中取消、TTFT 超时、连接断开和客户端重连的处理分支](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-api-engineering-streaming-exceptions.webp)
...
[原图：从生成幂等 Key、原子占位、调用模型到退避重试和最终落库的处理流程](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-api-engineering-retry-idempotency.webp)
...
[原图：JSON Mode 保证语法，JSON Schema 定义契约，Structured Outputs 在生成阶段应用契约](https://oss.javaguide.cn/github/javaguide/ai/llm/structured-output-function-calling-three-layer-constraint.png)
