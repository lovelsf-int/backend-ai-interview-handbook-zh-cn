---
title: "AI 核心概念总览：LLM、Agent、RAG、MCP 与系统设计 · 学习版"
description: "AI 核心概念总览：许可正文与个人复习"
status: "reviewing"
...
source_blob: "358f85f0d6f0ff60cfeb19867a2f7539c686e9af"
source_url: "https://javaguide.cn/ai/ai-core-concepts.html"
---

# AI 核心概念总览：LLM、Agent、RAG、MCP 与系统设计

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/ai-core-concepts.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/ai-core-concepts.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
先把模型、知识、工具与执行器放回各自的位置：LLM 生成候选输出，RAG 提供检索证据，工具产生外部效果，Agent 应用负责多步控制。

### 在脑中走一遍链路

用户目标 → 认证与上下文组装 → 检索或模型推理 → 工具调用提议 → 服务端校验与执行 → 观察结果 → 结束或继续。

### 容易记错的边界

不要把 Function Calling 说成模型直接执行 Java 方法；也不要把有检索的问答系统一律称为自治 Agent。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜RAG、微调和工具调用分别解决什么问题？</summary>

RAG 把可检索资料提供给生成过程；微调修改模型参数以影响行为；工具调用把结构化意图交给外部程序。三者可以组合，但不能互相替代。

</details>

<details>
<summary>P7｜SOC 研判为什么不能从一开始就做全自动 Agent？</summary>

先定义哪些步骤可确定化、哪些需要模型判断。可以从只读证据检索和建议开始，把高风险处置留在明确策略和审批之后，再根据评测开放自动化。

</details>

<details>
<summary>P8｜怎么证明加 Agent 比固定流程值得？</summary>

在同一任务集和权限边界下比较成功率、人工介入、尾延迟、成本与错误副作用。复杂度只有换来可验证收益才值得保留。

</details>

### 动手或纸上推演

用一个合成 SOC 告警分别设计规则流程、RAG 问答、受控 Agent 三个方案，写清每种方案不能完成的任务。

深入对照：[本站 P7/P8 工程章节](/ai-agent/01-llm-agent-basics.md)。官方/论文延伸：[Anthropic：Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)。
...
- [LLM 运行机制：Token、上下文窗口与采样参数怎么影响输出](/ai-study/java-guide/llm-basis/llm-operation-mechanism.md)
- [大模型 API 调用工程实践：流式输出、重试、限流与结构化返回](/ai-study/java-guide/llm-basis/llm-api-engineering.md)
- [大模型提示词工程（Prompt Engineering）是什么？提示词技巧有哪些？](/ai-study/java-guide/agent/prompt-engineering.md)
- [大模型结构化输出：从 JSON 契约到 Function Calling 落地](/ai-study/java-guide/llm-basis/structured-output-function-calling.md)
- [AI 应用评测体系：从 Golden Set 构建到线上灰度闭环](/ai-study/java-guide/llm-basis/llm-evaluation.md)
...
[原图：上下文窗口（Context Window）= LLM 的「工作记忆」](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-context-window.png)
...
[原图：Temperature 参数：控制模型输出的随机性](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-temperature-params.png)
...
[原图：Prompt 四要素框架](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/prompt-four-element-framework.svg)
...
[原图：生成阶段三层约束：JSON Mode 管语法，JSON Schema 管契约，Structured Outputs 把契约前移到模型生成阶段](https://oss.javaguide.cn/github/javaguide/ai/llm/structured-output-function-calling-three-layer-constraint.png)
...
[原图：Function Calling 完整调用链路：模型只生成调用意图，真正执行工具的是业务侧](https://oss.javaguide.cn/github/javaguide/ai/llm/structured-output-function-calling-function-calling-pipeline.png)
...
- [AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册](/ai-study/java-guide/agent/agent-basis.md)
- [AI 工作流中的 Workflow、Graph 与 Loop：从概念到实现](/ai-study/java-guide/agent/workflow-graph-loop.md)
- [上下文工程(Context Engineering) 是什么？和 Prompt Engineering 有什么区别？](/ai-study/java-guide/agent/context-engineering.md)
- [AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制](/ai-study/java-guide/agent/agent-memory.md)
- [多 Agent 协作系统设计：任务拆分、状态共享、冲突处理与失败恢复](/ai-study/java-guide/agent/multi-agent.md)
- [什么是 Model Context Protocol (MCP)？和 Function Calling、Agent 什么关系？](/ai-study/java-guide/agent/mcp.md)
- [Agent Skills 是什么？和 Prompt、MCP 到底差在哪？](/ai-study/java-guide/agent/skills.md)
- [Harness Engineering：六层检查框架、上下文管理与工程实践](/ai-study/java-guide/agent/harness-engineering.md)
- [Loop Engineering 是什么？为什么说它是新瓶装旧酒？](/ai-study/java-guide/agent/loop-engineering.md)
...
[原图：ReAct-LLM](https://oss.javaguide.cn/github/javaguide/ai/agent/ReAct-LLM.png)

落地 ReAct 时，运行时要保存执行历史和当前状态，向模型提供可用工具或 Skills，并把工具响应作为 Observation 关联到对应调用。系统告警、用户补充信息等环境变化，也要在后续轮次中更新。

[原图：ReAct 模式流程](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-react-flow.png)
...
[原图：Context Engineering 和 Prompt Engineering 差别](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/context-engineering-vs-context-engineering-dimension-comparison.png)

进一步的设计方法见[《提示词工程（Prompt Engineering）》](/ai-study/java-guide/agent/prompt-engineering.md)和[《上下文工程（Context Engineering）》](/ai-study/java-guide/agent/context-engineering.md)。
...
[原图：AI Agent 记忆系统架构](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-memory-arch.png)
...
[原图：长期记忆与 RAG（检索增强生成）的区别](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-memory-rag-vs-memory.svg)
...
[原图：MCP 图解](https://oss.javaguide.cn/github/javaguide/ai/skills/mcp-simple-diagram.png)
...
[原图：FC/MCP/Agent 三层关系图](https://oss.javaguide.cn/github/javaguide/ai/skills/mcp-fc-agent-layer.png)
...
[原图： Skill 和 Prompt、MCP、Function Calling 对比](https://oss.javaguide.cn/github/javaguide/ai/skills/skill-prompt-function-calling-mcp-comparison.webp)

Skill 的加载和执行包含五步：

[原图：Agent 执行链路](https://oss.javaguide.cn/github/javaguide/ai/skills/skill-agent-execution-link.webp)
...
[原图：Agent = Model + Harness](https://oss.javaguide.cn/github/javaguide/ai/harness/harness-agent-equals-model-harness-arch.png)
...
[原图：Loop Engineering 外层循环](https://oss.javaguide.cn/github/javaguide/ai/agent/loop-engineering-outer-loop.webp)
...
- [RAG 基础概念：检索、生成与工程取舍](/ai-study/java-guide/rag/rag-basis.md)
- [RAG 向量索引算法和向量数据库](/ai-study/java-guide/rag/rag-vector-store.md)
- [RAG 文档处理与切分策略：从解析、清洗、Chunking 到多模态内容处理](/ai-study/java-guide/rag/rag-document-processing.md)
- [RAG 优化：从召回、重排到上下文工程](/ai-study/java-guide/rag/rag-optimization.md)
- [GraphRAG：用图结构补充向量检索](/ai-study/java-guide/rag/graphrag.md)
- [RAG 知识库文档如何更新：增量更新、版本控制、去重与全量重建](/ai-study/java-guide/rag/rag-knowledge-update.md)
...
[原图：RAG 示意图](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-simplified-architecture-diagram.jpeg)
...
[原图：索引和检索阶段的简化流程图](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-rag-engineering-link.png)
...
4. 文档拆分（Chunking）：用文本分割器把文档切成较小片段。这一步要兼顾语义完整性、Embedding 模型输入长度、生成模型上下文窗口和召回粒度。Chunk 太大容易引入噪声，太小又可能丢上下文。拆分策略会直接影响召回质量，详细可以看 [RAG 文档处理篇](/ai-study/java-guide/rag/rag-document-processing.md)。
...
6. 存储到向量存储或索引系统：把嵌入向量、原始内容和对应元数据存入向量存储或向量索引系统，比如 Milvus、pgvector、Elasticsearch / OpenSearch 向量检索，或基于 Faiss 构建本地向量索引。向量数据库选型、索引算法和 pgvector 实践可以看 [RAG 向量库篇](/ai-study/java-guide/rag/rag-vector-store.md)。
...
[原图：Embedding：把文本映射到语义空间](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-2-embedding-map-text-to-semantic-space.png)
...
[原图：Embedding 和向量检索是什么关系？](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-embedding-vector-retrieval.png)
...
[原图：RAG 场景为什么需要向量数据库？](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-why-need-vector-store.png)

### 文档处理

文档从上传到进入向量库，中间要经过至少六个环节：

[原图：RAG 文档处理总链路：上传前半段决定了后半段效果上限](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-document-processing-overall-link.png)
...
[原图：如何选择合适的切分策略？](https://oss.javaguide.cn/github/javaguide/ai/rag/rag-document-processing-chunking-strategy.png)
...
[原图：什么是 GraphRAG？](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-simplified-architecture-diagram.png)
...
[原图：GraphRAG 和传统向量 RAG 的本质区别](https://oss.javaguide.cn/github/javaguide/ai/rag/graphrag-vs-rag.png)
...
- [AI 应用系统设计：从 Prompt Demo 到生产级架构](/ai-study/java-guide/system-design/ai-application-architecture.md)
- [大模型网关详解：多模型路由、Fallback、限流与成本控制](/ai-study/java-guide/system-design/llm-gateway.md)
- [AI 可观测性与 Trace：如何还原 Agent 的完整执行过程](/ai-study/java-guide/system-design/ai-observability.md)
- [LLM/Agent 安全实战：从 Prompt Injection、工具越权到沙箱隔离](/ai-study/java-guide/system-design/llm-security.md)
- [AI 语音技术详解：从 ASR、TTS 到实时语音 Agent 的工程化落地](/ai-study/java-guide/system-design/ai-voice.md)
