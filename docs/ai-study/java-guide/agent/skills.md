---
title: "Agent Skills 是什么？和 Prompt、MCP 到底差在哪？ · 学习版"
description: "Agent Skills：许可正文与个人复习"
status: "reviewing"
...
source_blob: "88de546e91b8d78e82f51ba28d97c73e8b206397"
source_url: "https://javaguide.cn/ai/agent/skills.html"
---

# Agent Skills 是什么？和 Prompt、MCP 到底差在哪？

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/skills.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/skills.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
Skill 把可复用流程、说明和配套资源组织成可发现的能力包；它与工具接入协议不同，也不是自动获得执行权限的授权书。

### 在脑中走一遍链路

发现名称与描述 → 判断任务匹配 → 加载正文 → 按需读取资源或执行受控脚本 → 校验结果 → 记录版本。

### 容易记错的边界

安装了 Skill 不代表引用的工具已经连接；allowed-tools 等字段的支持和强制执行取决于宿主，不能仅凭文件声明认为安全。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜Skill 与 MCP 的区别是什么？</summary>

Skill 主要表达如何完成一类任务及所需资源；MCP 主要规范如何发现并调用外部能力。一个 Skill 可以指导使用 MCP 工具，也可以只提供本地流程。

</details>

<details>
<summary>P7｜为什么采用渐进式加载？</summary>

先用简短元信息选择相关能力，再加载正文与必要资源，避免所有技能全文挤占上下文。还应控制资源路径和依赖边界。

</details>

<details>
<summary>P8｜第三方 Skill 升级怎样治理？</summary>

固定来源和版本，审核脚本、权限、外联和行为差异；升级走回归与灰度。即使描述没变，配套脚本也可能改变副作用，不能只审 Markdown 标题。

</details>

### 动手或纸上推演

为告警报告整理写一份学习用 Skill 目录设计，分别列出触发条件、不适用范围、输入契约、验证步骤与工具前提。

深入对照：[本站 P7/P8 工程章节](/ai-agent/04-tools-mcp-a2a.md)。官方/论文延伸：[Agent Skills 规范](https://agentskills.io/specification)。
...
[原图：SKILL.md 正文最好控制在 500 行以内](https://oss.javaguide.cn/github/javaguide/ai/skills/keep-skill-md-content-under-500-lines-for-best-performance.png)
...
[原图：查找自己需要和热门的 Skills](https://oss.javaguide.cn/github/javaguide/ai/skills/skillssh.png)

[原图：Superpowers 内置的 skills](https://oss.javaguide.cn/github/javaguide/ai/skills/superpowers-skills.png)
...
[原图：渐进式披露](https://oss.javaguide.cn/github/javaguide/ai/skills/skills-progressive-disclosure.svg)

就像查书一样。你不会先把整本书背下来，而是先看目录，确定章节，再翻到具体那一页。

一般可以分成三层：

[原图：渐进式披露（三层模型）](https://oss.javaguide.cn/github/javaguide/ai/skills/skills-progressive-disclosure-three-layer-model.png)
...
[原图：Skill 工作流设计](https://oss.javaguide.cn/github/javaguide/ai/skills/agent-skills-workflow-design.webp)
...
[原图：Skill 路由流程](https://oss.javaguide.cn/github/javaguide/ai/skills/agent-skills-routing-flow.webp)
...
[原图：Skill 路由流程](https://oss.javaguide.cn/github/javaguide/ai/skills/skills-router.svg)
