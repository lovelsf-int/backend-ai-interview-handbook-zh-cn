---
title: "AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 · 学习版"
description: "Agent 核心机制：许可正文与个人复习"
status: "reviewing"
...
source_blob: "b175b568772817a92fce5d0fc80022bf6effc85f"
...
> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/agent-basis.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/agent-basis.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
Agent 的重点是依据观察进行多步选择；工程实现必须控制状态、权限、预算与终止，不能只写一个无限 while 循环。

### 在脑中走一遍链路

读取任务状态 → 组装受限上下文 → 模型提出下一步 → 策略校验 → 工具执行 → 持久化观察 → 判断终止或转人工。

### 容易记错的边界

工具返回成功不等于用户目标已完成；模型声称完成也不等于真实系统状态已改变。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜Workflow 与 Agent 的主要区别是什么？</summary>

Workflow 的路径主要由应用预先定义；Agent 可以由模型根据观察动态选择动作。生产系统可以在固定流程内嵌入受限 Agent 节点。

</details>

<details>
<summary>P7｜Agent 一直调用同一个工具怎么办？</summary>

定义最大步数、总时限、累计成本与重复状态检测；检查工具是否返回有效增量信息，再结束、换确定性分支或转人工。

</details>

<details>
<summary>P8｜如何定义可审计的完成状态？</summary>

每类任务定义外部可验证后置条件，绑定任务、动作及证据版本，区分建议已生成、请求已发出和业务效果已确认。

</details>

### 动手或纸上推演

用状态表描述一个只读告警分析 Agent，包括正常结束、缺少证据、工具失败和预算耗尽四种出口。

深入对照：[本站 P7/P8 工程章节](/ai-agent/02-architecture-orchestration.md)。官方/论文延伸：[Anthropic：Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)。
...
**2022 年，ChatGPT 这类产品刚火的时候**，模型主要依据已有知识回答问题，不能主动调用外部工具，也不能自行完成操作。[Prompt Engineering](/ai-study/java-guide/agent/prompt-engineering.md) 是当时最重要的使用方式：把约束和上下文说清楚，输出才更稳定。
...
[MCP 协议](/ai-study/java-guide/agent/mcp.md)开始处理工具接入碎片化的问题，Computer Use 则把可执行范围扩展到图形界面。Cursor、Claude Code、Codex 等编程工具也逐渐把代码库阅读、修改、测试和提交串进同一条任务链路，“Vibe Coding”随之被更多人讨论。
...
Claude Code、Cursor 等工具会扫描项目中的 `.claude/skills/` 目录，由模型决定是否激活某个 Skill。调用路径固定时用 Toolkits；需要沉淀团队经验、又保留任务流程弹性时，Agent Skills 更合适。路由设计、`SKILL.md` 的写法和第三方 Skill 安全审计可参见：[《Agent Skills 详解》](/ai-study/java-guide/agent/skills.md)。
...
Prompt 是给大语言模型的指令与上下文。Prompt Engineering 要处理的是任务边界、输出格式和约束条件：缺少这些信息时，模型只能自行猜测；条件明确后，输出才有稳定的依据。具体方法见：[《提示词工程（Prompt Engineering）》](/ai-study/java-guide/agent/prompt-engineering.md)。
...
这块展开讲内容很多，可以单独看这篇：[《提示词工程（Prompt Engineering）》](/ai-study/java-guide/agent/prompt-engineering.md) 和 [《上下文工程（Context Engineering）》](/ai-study/java-guide/agent/context-engineering.md)。
...
[原图：Multi-Agent 系统架构](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-multi-agent-arch.png)

当任务确实能按专业角色拆分时，Multi-Agent 可以并行执行，且单个子任务失败未必阻断整体。代价是 Agent 间的通信、协调和调试成本都会上升，Token 消耗也随之增加。

落地时还要处理任务契约、共享状态、并行写冲突、Worker 接管和检查点恢复。详细设计可以看 [《多 Agent 协作系统设计：任务拆分、状态共享、冲突处理与失败恢复》](/ai-study/java-guide/agent/multi-agent.md)。
...
[原图：A2A 协议架构](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-a2a.png)

### Agentic Workflows

Agentic Workflows 是吴恩达（Andrew Ng）重点倡导的概念，强调用工程编排把推理、工具、记忆、反思和多实体协作接成可执行流程，而不只等待底层模型能力变化。

[原图：智能体工作流核心模式](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-agentic-workflows.png)
...
这套图结构比写死的 if-else 链更容易扩展，出了问题也好定位到哪个节点哪条边。LangGraph（Python）和 Spring AI Alibaba Graph（Java）都是基于这套思路实现的。详细设计和代码实现可以看：[《AI 工作流中的 Workflow、Graph 与 Loop》](/ai-study/java-guide/agent/workflow-graph-loop.md)。
