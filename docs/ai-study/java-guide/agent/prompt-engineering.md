---
title: "大模型提示词工程（Prompt Engineering）是什么？提示词技巧有哪些？ · 学习版"
description: "Prompt 工程：许可正文与个人复习"
status: "reviewing"
...
source_blob: "6f379e04429a2d663a35b4ef6b22b0bcd48c8d16"
source_url: "https://javaguide.cn/ai/agent/prompt-engineering.html"
---

# 大模型提示词工程（Prompt Engineering）是什么？提示词技巧有哪些？

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/prompt-engineering.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/prompt-engineering.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
提示词是可版本化的任务说明与输入组织方式。好提示词需要明确目标、约束、资料和输出要求，并接受测试而不是凭感觉调。

### 在脑中走一遍链路

定义任务 → 组织可信指令与不可信资料 → 示例与输出约束 → 固定样本评测 → 记录版本 → 回归与发布。

### 容易记错的边界

在 Prompt 中写不要越权，不会自动给下游 API 建立权限边界；增加角色描述也不等于增加真实能力。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜Few-shot 示例应优先教什么？</summary>

优先展示容易歧义的输入输出边界、格式和反例，而不是只堆相似正确样例。示例数量应受上下文成本和验证收益约束。

</details>

<details>
<summary>P7｜修好一类问题却损坏另一类，怎么处理？</summary>

按任务分层评测并保留回归样本；把修改作为版本变化比较，不直接覆盖生产提示词。必要时拆分任务或确定性规则。

</details>

<details>
<summary>P8｜Prompt 平台需要哪些治理能力？</summary>

模板与变量校验、版本和发布审批、权限、脱敏、回归、灰度、回滚与成本归因。模型和工具版本要能与 Prompt 版本关联重放。

</details>

### 动手或纸上推演

给同一告警写两个 Prompt，只改变一个因素；先写评测标准，再比较样例与错误，不以回答更长作为更好。

深入对照：[本站 P7/P8 工程章节](/ai-agent/03-prompt-context-engineering.md)。官方/论文延伸：[Anthropic：Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)。
...
> 前置知识：本文默认你已经理解 Token、上下文窗口、Temperature、Top-p 等 LLM 底层概念。如果还不熟，可以先看[《LLM 运行机制：Token、上下文窗口与采样参数怎么影响输出》](/ai-study/java-guide/llm-basis/llm-operation-mechanism.md)。
...
[原图：六大核心技巧](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/prompt-six-core-techniques.svg)
...
[原图：任务分解](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/task-decomposition.svg)
...
[原图：结构化输出格式对比](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/structured-output-formats.svg)
...
[原图：prompt-injection-protection-three-layer-defense-in-depth-system](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/prompt-injection-protection-three-layer-defense-in-depth-system.svg)
...
如果需要继续设计检索前 ACL、工具资源鉴权、MCP Token、参数绑定审批、代码隔离和安全回归，可以看 [LLM/Agent 安全实战](/ai-study/java-guide/system-design/llm-security.md)。
...
关于 Context Engineering 的详细介绍，推荐阅读这篇：[上下文工程(Context Engineering) 是什么？和 Prompt Engineering 有什么区别？](/ai-study/java-guide/agent/context-engineering.md)
