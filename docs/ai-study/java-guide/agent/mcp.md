---
title: "什么是 Model Context Protocol (MCP)？和 Function Calling、Agent 什么关系？ · 学习版"
description: "MCP 协议：许可正文与个人复习"
status: "reviewing"
...
source_blob: "c0ea347096545fb622d27838b1f35b8d9a36dfd6"
source_url: "https://javaguide.cn/ai/agent/mcp.html"
---

# 什么是 Model Context Protocol (MCP)？和 Function Calling、Agent 什么关系？

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/mcp.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/mcp.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
MCP 规范应用与外部能力之间的发现和调用，不替模型做判断，也不替业务系统做资源授权。阅读前先区分协议修订版。

### 在脑中走一遍链路

Host 管理 Client → 按共同协议版本发现能力 → 模型产生工具意图 → Client 请求 Server → Server 执行业务校验与工具 → 结果回到应用。

### 容易记错的边界

上游正文以 2025-11-25 为历史基线。核验到的官方 2026-07-28 文档采用无状态请求元数据和 server/discover，并弃用 sampling；不能把两版初始化流程混用。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜模型究竟在哪里调用了 MCP？</summary>

模型通常只输出结构化工具意图，Host 中的运行时代码使用 MCP Client 发送协议请求。Server 再访问实际系统；模型本身不是 HTTP 客户端。

</details>

<details>
<summary>P7｜MCP 工具接通但调用被拒绝，查什么？</summary>

分别查版本与能力兼容、传输、认证、令牌受众/范围、工具 Schema 和业务对象权限。协议成功不等于业务操作被授权。

</details>

<details>
<summary>P8｜升级 MCP 时如何避免知识和实现混乱？</summary>

为 Host、Client、Server、SDK 建版本与能力矩阵，固定契约测试；按实际支持版本走发现与调用流程，不从最新文章推定旧客户端已支持新协议。

</details>

### 动手或纸上推演

画出模型输出、Host 调度、Client 请求、Server 业务执行四个边界，分别写出一条日志和一个失败样例。

深入对照：[本站 P7/P8 工程章节](/ai-agent/04-tools-mcp-a2a.md)。官方/论文延伸：[MCP 官方架构文档（2026-07-28）](https://modelcontextprotocol.io/docs/2026-07-28/learn/architecture)。
...
> 本文保留上游写作时的 [2025-11-25 revision](https://modelcontextprotocol.io/specification/2025-11-25) 为主。2025-03-26 版本把早期 HTTP+SSE 传输调整为 Streamable HTTP，2025-06-18 加入 Elicitation，2025-11-25 又增加了实验性的 Tasks、URL 模式 Elicitation 等内容。客户端和 SDK 可能只实现其中一部分，接入前要同时确认协议 revision、SDK 版本和 Host 能力。
...
[原图：MCP 四层架构](https://oss.javaguide.cn/github/javaguide/ai/skills/mcp-four-layer-architecture.png)
...
[原图：MCP 调用时序图](https://oss.javaguide.cn/github/javaguide/ai/skills/mcp-call-seq.png)
...
[原图：MCP 传输方式选择](https://oss.javaguide.cn/github/javaguide/ai/skills/mcp-transport-decision.png)
...
Prompt Injection、Token Passthrough、资源级鉴权、本地 Server 隔离和第三方 Skill/MCP 供应链的完整链路，可以继续看 [LLM/Agent 安全实战](/ai-study/java-guide/system-design/llm-security.md)。
