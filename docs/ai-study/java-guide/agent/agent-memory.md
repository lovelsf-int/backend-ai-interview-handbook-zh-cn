---
title: "AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 · 学习版"
description: "Agent 记忆系统：许可正文与个人复习"
status: "reviewing"
...
source_blob: "f0b4f901cd391a8c9ceb68166dfd6447b0869c9d"
source_url: "https://javaguide.cn/ai/agent/agent-memory.html"
---

# AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/agent-memory.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/agent-memory.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
记忆是有来源、用途、作用域和生命周期的数据。对话记录、摘要、任务状态和长期知识不能混用一套写入规则。

### 在脑中走一遍链路

候选记忆 → 来源与权限校验 → 去重/冲突处理 → 存储与版本 → 按任务检索 → 过期、撤权或删除。

### 容易记错的边界

用户一次表达、模型推断和已确认事实可信度不同；不能把工具返回的恶意指令自动写成长期规则。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜短期上下文和长期记忆差别在哪里？</summary>

短期上下文服务当前会话推理；长期记忆跨会话复用，需要独立的身份隔离、持久化、更新和删除规则。长期存在不意味着每次都应放入 Prompt。

</details>

<details>
<summary>P7｜用户说之前记错了，如何纠正？</summary>

保存可追溯修订，明确生效版本；失效检索缓存与派生摘要，必要时从权威事实重新生成，避免旧记忆继续被召回。

</details>

<details>
<summary>P8｜多租户共享记忆服务如何避免污染？</summary>

从认证层固定作用域，区分个人、团队与公共知识；各层缓存和摘要继承权限，写入有来源、审核与撤销链路，并持续测试越权读取和跨域写入。

</details>

### 动手或纸上推演

设计一条记忆的数据结构，至少包含主体、租户、来源、置信依据、用途、版本、有效期和删除状态。

深入对照：[本站 P7/P8 工程章节](/ai-agent/07-memory-session-personalization.md)。官方/论文延伸：[LangGraph：Persistence](https://docs.langchain.com/oss/python/langgraph/persistence)。
...
记忆层需要同时保住当前对话的关键事实，并让新 Session 能取回用户偏好、背景和历史决策。文章依次讨论记忆的表征和功能分类、读写生命周期、短期与长期实现、主流产品和检索优化，以及 Markdown 记忆。滑动窗口怎么裁、overload 怎么卸，和同站的 [《上下文工程(Context Engineering) 是什么？和 Prompt Engineering 有什么区别？》](/ai-study/java-guide/agent/context-engineering.md) 有交集，两篇可以对着看。
...
[原图：记忆操作的生命周期](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-memory-lifestyle.png)
...
[原图：记忆系统的高级演化机制](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-memory-evolution.png)
...
[原图：长期记忆的检索优化策略](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-memory-retrieval-optimization.png)
...
[原图：Markdown 记忆和传统长期记忆的适用边界](https://oss.javaguide.cn/github/javaguide/ai/agent/agent-memory-markdown-memory-boundary.svg)
