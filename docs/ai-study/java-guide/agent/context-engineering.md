---
title: "上下文工程(Context Engineering) 是什么？和 Prompt Engineering 有什么区别？ · 学习版"
description: "Context Engineering：许可正文与个人复习"
status: "reviewing"
...
source_blob: "fc69ec5b32e5e85760fe98a211526eee55aa787e"
source_url: "https://javaguide.cn/ai/agent/context-engineering.html"
---

# 上下文工程(Context Engineering) 是什么？和 Prompt Engineering 有什么区别？

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/context-engineering.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/context-engineering.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
上下文工程决定这一轮模型应该看到哪些信息，以及怎样排序、压缩、更新与隔离。不是把历史消息全部拼起来。

### 在脑中走一遍链路

确定任务与权限 → 获取当前事实和证据 → 去重排序 → 预留输出/工具预算 → 压缩次要内容 → 记录上下文版本。

### 容易记错的边界

压缩可能丢掉否定词、时间条件和授权边界；摘要不能替代外部系统中的权威任务状态。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜Prompt 工程与上下文工程如何区分？</summary>

Prompt 更关注指令与表达；上下文工程还包括动态资料、工具结果、记忆、预算与生命周期。两者交叉，但后者覆盖更大的数据流。

</details>

<details>
<summary>P7｜上下文快满时先删什么？</summary>

先按任务相关性、可信度、时效和重复性处理资料，保护硬约束、当前状态和必要证据。不能简单删除最早消息或截断最后几个字符。

</details>

<details>
<summary>P8｜如何评估压缩器是否安全？</summary>

建立包含否定、条件、版本、权限和矛盾证据的样本，比较压缩前后结论与引用支撑；关键规则使用结构化状态保存而不依赖有损摘要。

</details>

### 动手或纸上推演

把一段含过期 SOP、最新 SOP 和案例的上下文压缩到原长度的一半，逐条核对约束是否保留。

深入对照：[本站 P7/P8 工程章节](/ai-agent/03-prompt-context-engineering.md)。官方/论文延伸：[Anthropic：Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)。
...
[原图：以电商售后为例图解同样的 Agent，为什么表现差这么多](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/why-the-same-agent-performs-so-differently.png)
...
[原图：Prompt vs Context 工程维度对比](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/prompt-vs-context-engineering-dimension-comparison.svg)
...
[原图：上下文窗口（Context Window）= LLM 的工作记忆](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-context-window.png)
...
[原图：上下文为什么会失效](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/why-does-the-following-content-fail.png)

窗口容量增加后，筛选问题仍然存在。输入超出当前任务所需范围时，额外材料可能只会增加干扰。

[原图：上下文利用率的 40% 阈值现象](https://oss.javaguide.cn/github/javaguide/ai/harness/context-utilization-40-percent-threshold-phenomenon.svg)
...
[原图：上下文腐化](https://oss.javaguide.cn/github/javaguide/ai/harness/context-rot-diagram.png)
...
[原图：运行时上下文怎么检索](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/context-engineering-run-time-retrieval.png)
...
Anthropic 将这类分层获取信息的方式称为 **Progressive Disclosure**，即渐进式披露。Agent 通过多轮探索补充上下文：文件大小提示复杂度，时间戳提示相关性，目录结构提供位置语义。Skills 也利用了这一思路，具体可见：[Agent Skills 是什么？和 Prompt、MCP 到底差在哪？](/ai-study/java-guide/agent/skills.md)。
...
[原图：长任务上下文持久化：抵抗腐化的三大武器](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/long-task-context-persistence-three-weapons-against-corruption.svg)
...
[原图： Claude Code 的上下文压缩思路](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/claude-code-context-compression-thinking.png)
...
[原图：Sub-agent 拆分任务，隔离上下文](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/sub-agent-task-splitting-context-isolation%20.png)
...
[原图：上下文工程过程中的系统提示](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/calibrating-the-system-prompt.png)
...
[原图：上下文不是越多越好](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/context-engineering-eviction-strategy.png)
...
[原图：Context Engineering 的核心逻辑](https://oss.javaguide.cn/github/javaguide/ai/context-engineering/context-engineering-core-logic.png)
