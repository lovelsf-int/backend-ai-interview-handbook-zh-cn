---
title: "Loop Engineering 是什么？为什么说它是新瓶装旧酒？ · 学习版"
description: "Loop Engineering：许可正文与个人复习"
status: "reviewing"
...
source_blob: "0dd90b03877852922080e96d634fc08e0bb9c3f4"
source_url: "https://javaguide.cn/ai/agent/loop-engineering.html"
---

# Loop Engineering 是什么？为什么说它是新瓶装旧酒？

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/loop-engineering.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/loop-engineering.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
循环工程的价值不在循环本身，而在每一轮输入、验证、进展和停止条件都可解释、可控制。

### 在脑中走一遍链路

触发 → 读取真实状态 → 选择一个可验收目标 → 执行 → 验证增量 → 保存进展 → 停止、继续或升级处理。

### 容易记错的边界

失败后原样重试不一定产生新信息；预算只写在 Prompt 里也不能阻止运行时继续调用。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜一个可靠循环至少需要什么？</summary>

状态、动作、观察、验证与停止条件，并有外部强制的轮数、时间和成本约束。必须知道这一轮相对上一轮改变了什么。

</details>

<details>
<summary>P7｜连续几轮无进展怎么判定？</summary>

比较规范化任务状态、动作参数和验证结果，结合失败类型判断；记录无进展原因，进入确定性修复、停止或人工，而不是无限重复同一计划。

</details>

<details>
<summary>P8｜多任务循环如何避免预算失控？</summary>

使用父任务累计预算、子任务配额和并发隔离，合并所有尝试的成本；任务转交或重新规划不得重新获得完整预算。

</details>

### 动手或纸上推演

为循环列出六个停止原因，并为每个原因定义用户可见状态、审计字段与是否允许自动恢复。

深入对照：[本站 P7/P8 工程章节](/ai-agent/06-planning-execution-recovery.md)。官方/论文延伸：[Anthropic：Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)。
...
[AI Agent 基础概念](/ai-study/java-guide/agent/agent-basis.md) 中介绍过这条循环。线上故障排查、代码库阅读和测试失败定位都没有预先确定的完整路径，模型需要根据每次拿到的证据调整下一步。
...
[原图：Workflow、Graph、Loop 三者关系概览](https://oss.javaguide.cn/github/javaguide/ai/workflow/workflow-graph-loop-relation.svg)

“生成初稿 → 审核 → 不通过就修改 → 再审核”中，审核不通过的条件边就是从“审核”回到“修改”的回边；审核通过则离开循环。 [AI 工作流中的 Workflow、Graph 与 Loop](/ai-study/java-guide/agent/workflow-graph-loop.md) 对这套结构有更完整的说明。运行配置还要写明最大轮次、超时、Token 预算和失败后的降级方式，防止回边没有出口。
...
在 [Harness Engineering](/ai-study/java-guide/agent/harness-engineering.md) 中，Agent 可以拆成 Model + Harness。模型负责推理和生成，Harness 提供环境、工具、反馈、沙箱、权限、观测和恢复。
...
[原图：Agent 执行链路](https://oss.javaguide.cn/github/javaguide/ai/skills/skills-agent-execution-link.png)
...
[原图：Skill 渐进式披露](https://oss.javaguide.cn/github/javaguide/ai/skills/agent-skills-progressive-disclosure.webp)
...
[原图：Claude Code 推荐使用 loop 命令](https://oss.javaguide.cn/github/javaguide/ai/coding/claudecode/claudecode-father-loop.png)
...
[原图：CI 排查 Loop 示例](https://oss.javaguide.cn/github/javaguide/ai/agent/loop-engineering-ci-triage-loop.webp)
...
[原图：Loop 的安全边界](https://oss.javaguide.cn/github/javaguide/ai/agent/loop-engineering-safety-boundary.webp)
