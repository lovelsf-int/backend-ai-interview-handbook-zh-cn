---
title: "Harness 是什么？Harness Engineering 六层架构与 Agent 工程实践 · 学习版"
description: "Harness Engineering：许可正文与个人复习"
status: "reviewing"
...
source_blob: "45991f5a076bc4b2c72fc131a54e972cd834f482"
source_url: "https://javaguide.cn/ai/agent/harness-engineering.html"
---

# Harness 是什么？Harness Engineering 六层架构与 Agent 工程实践

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/agent/harness-engineering.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/agent/harness-engineering.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
Harness 是围绕模型的执行与反馈环境：它管理工具、上下文、状态、验证和恢复，使长任务不只依赖一次模型回答。

### 在脑中走一遍链路

初始化环境 → 拆分可验收任务 → 执行增量步骤 → 运行验证 → 保存真实进展与工件 → 下一轮读取并继续。

### 容易记错的边界

工作日志不是完成证明；只保存模型总结却不保存代码、测试和状态，恢复时仍会重复猜测。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜Harness 与模型能力是什么关系？</summary>

模型负责生成与判断，Harness 决定可见上下文、可用工具、执行边界和反馈。换更强模型不能自动修复缺少验证或状态持久化的问题。

</details>

<details>
<summary>P7｜上下文换新后如何接续任务？</summary>

读取有版本的任务状态、工件、最近验证结果和未决事项，先核验现场再继续；不能直接相信上一轮自述已完成。

</details>

<details>
<summary>P8｜怎么衡量 Harness 改进？</summary>

固定模型和任务集，比较长任务完成率、重复工作、恢复成功率、错误外部动作与累计成本；把改进归因到具体状态和验证机制。

</details>

### 动手或纸上推演

设计一次中断恢复包：任务清单、工件版本、已通过验证、未决问题、当前权限与剩余预算。
...
[原图：Harness Engineering 六层架构](https://oss.javaguide.cn/github/javaguide/ai/harness/harness-engineering-six-layer-architecture.svg)
...
Agent Skills 也采用了相同的渐进式披露：上下文中常驻名称、描述等元数据，命中场景后再加载详细规则和执行流程。它把 `AGENTS.md` 的目录式做法标准化了。相关阅读可以看这篇：[Agent Skills 详解：是什么？怎么用？和 Prompt、MCP 有什么区别？](/ai-study/java-guide/agent/skills.md)。
...
[原图：Anthropic 三智能体协同架构（受 GAN 启发）](https://oss.javaguide.cn/github/javaguide/ai/harness/anthropic-three-agent-collaborative-architecture-inspired-by-gan.svg)
...
[原图：Stripe 混合状态机编排架构](https://oss.javaguide.cn/github/javaguide/ai/harness/stripe-hybrid-state-machine-orchestration-architecture.svg)
...
[原图：持续进化的 Harness 防错反馈闭环](https://oss.javaguide.cn/github/javaguide/ai/harness/continuously-evolving-harness-error-prevention-feedback-loop.svg)
