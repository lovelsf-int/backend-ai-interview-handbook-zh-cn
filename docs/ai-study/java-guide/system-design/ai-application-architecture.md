---
title: "AI 应用系统设计：从 Prompt Demo 到生产级架构 · 学习版"
description: "AI 应用架构：许可正文与个人复习"
status: "reviewing"
...
source_blob: "532aa27142089cbf6f9e9d258057e798dadda198"
source_url: "https://javaguide.cn/ai/system-design/ai-application-architecture.html"
---

# AI 应用系统设计：从 Prompt Demo 到生产级架构

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/system-design/ai-application-architecture.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/system-design/ai-application-architecture.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
把概率生成放进有身份、状态、预算和恢复能力的后端系统。架构设计应先明确质量、延迟与错误代价，再选择框架。

### 在脑中走一遍链路

入口鉴权 → 任务与预算 → 检索/上下文 → 模型与工具 → 业务策略 → 状态持久化 → 结果投影、审计与评测。

### 容易记错的边界

一次演示成功不能证明高可用；模型返回、队列投递、ES 写入和业务处置不是同一个完成状态。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜AI 应用比普通 API 服务新增了哪些关注点？</summary>

需要管理不确定输出、上下文和模型版本、质量评测、Token 成本及工具副作用，同时仍然保留传统的权限、可靠性和数据治理。

</details>

<details>
<summary>P7｜同步接口与异步任务怎样划分？</summary>

依据用户交互和任务时长拆分，长任务持久化状态并返回可查询任务标识；取消、超时、恢复与重复请求都要有明确语义。

</details>

<details>
<summary>P8｜如何避免 AI 平台成为巨大共享故障域？</summary>

按租户和能力隔离配额、并发、工具权限与队列，区分控制服务和数据处理；对关键依赖定义可验证的降级边界与容量证据。

</details>

### 动手或纸上推演

以合成 SOC 场景画出同步研判与异步处置链路，标出每个完成状态的权威存储和失败恢复责任。
...
如果你对大模型 API 的调用链还不熟，可以先看 [大模型 API 调用工程实践：流式输出、重试、限流与结构化返回](/ai-study/java-guide/llm-basis/llm-api-engineering.md)。如果是想补 Token、上下文窗口和采样参数这些基础，再看 [LLM 运行机制：Token、上下文窗口与采样参数怎么影响输出](/ai-study/java-guide/llm-basis/llm-operation-mechanism.md)。
...
模型网关负责统一接入 OpenAI、Anthropic、Google Gemini、私有化模型、Embedding 模型、Rerank 模型等能力。它隐藏不同 API 的差异，对上提供稳定接口。模型网关本身可以单独展开一篇，细节可以看 [大模型网关详解：多模型路由、Fallback、限流与成本控制](/ai-study/java-guide/system-design/llm-gateway.md)。

[原图：LLM 网关示意图](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-gateway-overview.png)
...
Prompt 写法本身可以看 [大模型提示词工程（Prompt Engineering）是什么？提示词技巧有哪些？](/ai-study/java-guide/agent/prompt-engineering.md)。如果你关心的是“哪些信息该进上下文、进多少、什么时候压缩”，更适合看 [上下文工程（Context Engineering）是什么？和 Prompt Engineering 有什么区别？](/ai-study/java-guide/agent/context-engineering.md)。
...
RAG 的基础概念可以从 [RAG 基础概念：检索、生成与工程取舍](/ai-study/java-guide/rag/rag-basis.md) 看起；文档如何解析、清洗和切 Chunk，可以看 [RAG 文档处理与切分策略](/ai-study/java-guide/rag/rag-document-processing.md)；检索效果调优看 [RAG 优化：从召回、重排到上下文工程](/ai-study/java-guide/rag/rag-optimization.md)。Memory 单独展开的话，可以看 [AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制](/ai-study/java-guide/agent/agent-memory.md)。
...
流式输出、重试、限流和结构化返回在 [大模型 API 调用工程实践](/ai-study/java-guide/llm-basis/llm-api-engineering.md) 里有更完整的工程拆解。如果场景是实时语音，还要考虑 VAD、ASR、TTS、打断和端到端延迟，可以继续看 [AI 语音技术详解：从 ASR、TTS 到实时语音 Agent 的工程化落地](/ai-study/java-guide/system-design/ai-voice.md)。
...
如果 Prompt 输出要被程序稳定解析，最好不要只靠“请返回 JSON”。结构化输出、JSON Schema、Function Calling 的工程细节可以看 [大模型结构化输出：从 JSON 契约到 Function Calling 落地](/ai-study/java-guide/llm-basis/structured-output-function-calling.md)。
...
这里的 Token 预算和上下文压缩，和前面提到的 Context Engineering 是同一类问题。更完整的上下文装配、按需加载和降级策略，可以看 [上下文工程（Context Engineering）是什么？和 Prompt Engineering 有什么区别？](/ai-study/java-guide/agent/context-engineering.md)。
...
工具调用这块如果想从概念补起，可以先看 [大模型结构化输出：从 JSON 契约到 Function Calling 落地](/ai-study/java-guide/llm-basis/structured-output-function-calling.md)。如果你的工具要被多个模型、Agent 或 IDE 复用，再看 [什么是 Model Context Protocol（MCP）？和 Function Calling、Agent 什么关系？](/ai-study/java-guide/agent/mcp.md)。
...
[原图：工具调用安全风险分层：按风险等级匹配不同的控制策略](https://oss.javaguide.cn/github/javaguide/ai/llm/structured-output-function-calling-tool-call-security.png)
...
知识库不是一次导入就结束。文档版本、增量同步、去重、回滚和全量重建都会影响线上答案，具体可以看 [RAG 知识库文档如何更新：增量更新、版本控制、去重与全量重建](/ai-study/java-guide/rag/rag-knowledge-update.md)。如果问题需要跨文档关系、实体关系和全局摘要，传统向量检索不一定够用，可以继续看 [GraphRAG：用图结构补充向量检索](/ai-study/java-guide/rag/graphrag.md)。
...
评测要有固定样本、明确的通过条件和可回放的目标版本。如何准备评测集、拆分 RAG 与 Agent 指标，以及把 LLM-as-Judge 接入 CI，可参见 [AI 应用评测体系](/ai-study/java-guide/llm-basis/llm-evaluation.md)。
...
Prompt 注入、上下文分区和工具权限其实是连在一起的，前面提到的 [Prompt Engineering](/ai-study/java-guide/agent/prompt-engineering.md)、[Context Engineering](/ai-study/java-guide/agent/context-engineering.md) 和 [MCP](/ai-study/java-guide/agent/mcp.md) 这几篇可以配合看。

如果系统已经允许 Agent 读取外部内容并调用写工具，还需要继续设计资源级鉴权、参数绑定审批、MCP Token、代码与网络隔离以及安全回归，详见 [LLM/Agent 安全实战](/ai-study/java-guide/system-design/llm-security.md)。
...
如果想从 Java 后端调用大模型 API 的细节入手，可以先看 [大模型 API 调用工程实践](/ai-study/java-guide/llm-basis/llm-api-engineering.md)；如果团队准备把模型调用统一成基础设施，建议把 [大模型网关详解](/ai-study/java-guide/system-design/llm-gateway.md) 单独读一遍。
...
如果你是按面试路线复习，可以直接看 [AI 系统设计面试题总结](/ai-study/java-guide/interview-questions/ai-system-design-interview-questions.md)。RAG、Agent 和大模型基础也分别有 [RAG 面试题总结](/ai-study/java-guide/interview-questions/rag-interview-questions.md)、[AI Agent 面试题总结](/ai-study/java-guide/interview-questions/agent-interview-questions.md) 和 [大模型基础面试题总结](/ai-study/java-guide/interview-questions/llm-interview-questions.md)。
...
- [AI 应用开发知识体系：大模型、Agent、RAG、MCP、Prompt 工程与系统设计](/ai-study/java-guide/index.md)
- [AI 系统设计专题：生产级架构、模型网关、评测治理与语音 Agent](/ai-study/java-guide/system-design/index.md)
- [大模型基础专题：运行机制、API 调用、结构化输出与评测](/ai-study/java-guide/llm-basis/index.md)
- [RAG 专题：文档处理、向量数据库、GraphRAG、检索优化与知识库更新](/ai-study/java-guide/rag/index.md)
- [AI Agent 专题：Agent Loop、Memory、Prompt、Context、MCP 与 Skills](/ai-study/java-guide/agent/index.md)
