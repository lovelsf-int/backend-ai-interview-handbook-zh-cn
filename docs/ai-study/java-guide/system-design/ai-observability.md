---
title: "AI 可观测性与 Trace：如何还原 Agent 的完整执行过程 · 学习版"
description: "AI 可观测性：许可正文与个人复习"
status: "reviewing"
...
source_blob: "af7ddae8d36138a64259ed63f6d4becff717092a"
source_url: "https://javaguide.cn/ai/system-design/ai-observability.html"
---

# AI 可观测性与 Trace：如何还原 Agent 的完整执行过程

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/system-design/ai-observability.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/system-design/ai-observability.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
可观测性帮助回答哪里慢、哪里贵、哪里错；质量评测回答是否完成任务。两者互补，但不能用 Trace 数量代替质量证据。

### 在脑中走一遍链路

任务关联 ID → 检索/模型/工具分段 Trace → 脱敏事件与指标 → 错误归因 → 受控回放 → 回归数据集。

### 容易记错的边界

全量记录 Prompt 和工具结果可能泄露数据；请求成功状态不代表回答正确或动作已经确认。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜一个 Agent Trace 要关联哪些版本？</summary>

至少关联任务、模型、Prompt、知识或索引、工具和策略版本，以及重试与父子任务关系。敏感内容以受控引用替代普通日志全文。

</details>

<details>
<summary>P7｜P99 突然上升，如何分段定位？</summary>

先区分排队、检索、模型首字、生成、工具和重试耗时，再按租户、模型、任务类型及长度分层，避免总体均值掩盖热点。

</details>

<details>
<summary>P8｜如何在隐私限制下支持问题复现？</summary>

保存必要元数据和版本，使用脱敏/合成样本与受控短期证据仓，建立访问审计；回放默认隔离写工具，不能重新触发生产动作。

</details>

### 动手或纸上推演

为一次研判设计 Trace 字段表，逐项写明是否敏感、是否采样、保留周期由谁批准、以及如何用于定位问题。

深入对照：[本站 P7/P8 工程章节](/ai-agent/10-evaluation-observability.md)。官方/论文延伸：[OpenTelemetry：GenAI semantic conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/)。
...
[原图：AI 可观测信号各自解决的问题](https://oss.javaguide.cn/github/javaguide/ai/llm/ai-observability-signals-overview.webp)
...
[原图：Session、Run、Trace、Span 和 Attempt 的层级关系](https://oss.javaguide.cn/github/javaguide/ai/llm/ai-observability-id-hierarchy.webp)
...
[原图：一次 Agent 请求的 Span 拆分示意图](https://oss.javaguide.cn/github/javaguide/ai/llm/ai-observability-agent-span-trace.webp)
...
[原图：Trace 上下文跨线程、进程和消息边界的传播方式](https://oss.javaguide.cn/github/javaguide/ai/llm/ai-observability-context-propagation.webp)
...
[原图：Trace 头部采样和尾部采样的区别](https://oss.javaguide.cn/github/javaguide/ai/llm/ai-observability-head-tail-sampling.webp)
...
关于 Golden Set、Agent 评测、线上 Badcase 回流和回归门禁，可以继续阅读[《大模型应用如何评测？》](/ai-study/java-guide/llm-basis/llm-evaluation.md)。
