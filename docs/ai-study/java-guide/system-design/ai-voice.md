---
title: "AI 语音技术详解：从 ASR、TTS 到实时语音 Agent 的工程化落地 · 学习版"
description: "实时语音 AI：许可正文与个人复习"
status: "reviewing"
...
source_blob: "94c3d5417f2857d53966d3ea0596857c4b577d86"
source_url: "https://javaguide.cn/ai/system-design/ai-voice.html"
---

# AI 语音技术详解：从 ASR、TTS 到实时语音 Agent 的工程化落地

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/system-design/ai-voice.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/system-design/ai-voice.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
实时语音需要联合管理识别、生成、合成、播放与打断状态。低延迟不只是换一个更快的语言模型。

### 在脑中走一遍链路

音频输入 → 语音活动/分段判断 → 识别或音频模型 → 推理与工具 → 合成/音频输出 → 播放 → 打断与状态更新。

### 容易记错的边界

用户打断后，仅停止扬声器不代表旧生成、工具或缓冲区都取消了；旧音频不能混入新一轮回答。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜级联 ASR/LLM/TTS 与端到端音频如何取舍？</summary>

从控制性、可观测、延迟、识别/生成质量、成本与业务工具集成比较。没有一种方式在所有场景都更优。

</details>

<details>
<summary>P7｜如何处理 barge-in 打断？</summary>

用轮次标识关联识别、生成、合成和播放，取消可取消任务并清理过期缓冲，已发生的业务副作用另行核验，不能视为一起回滚。

</details>

<details>
<summary>P8｜语音系统的质量门禁如何设计？</summary>

覆盖噪声、口音、长停顿、重叠说话、打断和网络抖动，测端到端任务成功与延迟分位数，同时验证工具安全和错误恢复。

</details>

### 动手或纸上推演

写一个对话轮次状态表，演练用户连续两次打断时哪些任务停止、哪些结果丢弃、哪些动作必须对账。
