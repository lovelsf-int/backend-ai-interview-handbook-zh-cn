---
title: "LLM/Agent 安全实战：从 Prompt Injection、工具越权到沙箱隔离 · 学习版"
description: "LLM 与 Agent 安全：许可正文与个人复习"
status: "reviewing"
...
source_blob: "8c07d3b742c4eed1499a5382618de6feb47e6ae1"
source_url: "https://javaguide.cn/ai/system-design/llm-security.html"
---

# LLM/Agent 安全实战：从 Prompt Injection、工具越权到沙箱隔离

> 来源：JavaGuide / Guide 与原贡献者。[原文](https://javaguide.cn/ai/system-design/llm-security.html) · [固定源码](https://github.com/Snailclimb/JavaGuide/blob/d76264cb4e000416c4adca06770ce014bd309150/docs/ai/system-design/llm-security.md) · [Apache-2.0 与修改说明](/ai-study/sources.md)。本站于 2026-09-09 增加学习卡并适配排版、链接与媒体引用。
...
安全目标是即使模型被误导，也不能突破外部身份、数据和执行权限。检测器与 Prompt 约束只是防御中的一部分。

### 在脑中走一遍链路

不可信资料 → 受限上下文 → 模型提议 → 资源与参数授权 → 绑定动作审批 → 幂等执行 → 对账与审计。

### 容易记错的边界

内部邮件、日志和案例仍可能含攻击者控制文本；通过 JSON 校验或模型自检不等于得到合法授权。

### 闭卷自测：先回答，再展开

<details>
<summary>基础｜直接和间接提示注入有什么差别？</summary>

入口不同：前者来自用户指令，后者藏在文档、网页或工具结果中。无论入口如何，不可信内容都不能修改服务端授权上下文。

</details>

<details>
<summary>P7｜审批后资产或参数被改了怎么办？</summary>

审批绑定具体动作、目标、规范化参数摘要、版本和有效期。执行前核对并重新授权，关键竞态需要下游条件执行或明确剩余风险。

</details>

<details>
<summary>P8｜怎么验证防线不只是一句提示词？</summary>

构造跨租户、审批篡改、工具结果注入和结果未知等测试，观察真实副作用、出口和状态；同时测正常任务误拒绝，不能靠全部拒绝宣称成功。

</details>

### 动手或纸上推演

先阅读本站 SOC 安全实战，再选择其中三个验收场景，写出确定性的断言而不是只判断模型是否说了拒绝。

深入对照：[本站 P7/P8 工程章节](/ai-agent/14-llm-agent-security-practice.md)。官方/论文延伸：[OWASP：AI Agent Security](https://cheatsheetseries.owasp.org/cheatsheets/AI_Agent_Security_Cheat_Sheet.html)。
...
[原图：一次 Agent 请求中的信任区](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-security-request-trust-boundaries.webp)
...
[原图：Prompt Injection、间接注入和 Jailbreak 的区别](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-security-prompt-injection-types.webp)
...
[原图：MCP Token 的受众和凭证边界](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-security-mcp-token-boundaries.webp)
...
[原图：高风险工具的安全执行链](https://oss.javaguide.cn/github/javaguide/ai/llm/llm-security-safe-tool-execution-chain.webp)
...
- [大模型提示词工程](/ai-study/java-guide/agent/prompt-engineering.md)
- [大模型结构化输出与 Function Calling](/ai-study/java-guide/llm-basis/structured-output-function-calling.md)
- [MCP 详解](/ai-study/java-guide/agent/mcp.md)
- [Agent Skills 详解](/ai-study/java-guide/agent/skills.md)
- [AI 应用系统设计](/ai-study/java-guide/system-design/ai-application-architecture.md)
- [AI 应用评测体系](/ai-study/java-guide/llm-basis/llm-evaluation.md)
